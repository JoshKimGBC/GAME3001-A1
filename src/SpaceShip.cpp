...
void SpaceShip::m_Move()
{
	...
	void SpaceShip::Seek() {
		...
		m_targetDirection = Util::normalize(m_destination - m_shipCenter);
		LookWhereYourGoing();
	}
	...
	void SpaceShip::Arrive() {
		...
		m_targetDirection = Util::normalize(m_destination - m_shipCenter);
		auto distanceToTarget = Util::distance(m_shipCenter, m_destination);

		if (distanceToTarget < m_arriveDistance)
			getRigidBody()->velocity *= 1;
